...
def construct_unique_key(baseurl, params):
    ...
    key_value_string = ""
    url_string = str(baseurl)
    for key in params:
        key_value_string_1 = "_"+str(key)+"_"+str(params[key])
        key_value_string = key_value_string+key_value_string_1
    unique_key = url_string+key_value_string
    return unique_key
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    search_keys = construct_unique_key(baseurl, params)
    if search_keys in CACHE_DICT:
        print("fetching cached data")
        return CACHE_DICT[search_keys]
    else:
        print("making new request")
        CACHE_DICT[search_keys] = make_request(baseurl, params)
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    #print(tags)
        for tag in tags:
            if "#"+str.lower(tag["text"]) == str.lower(hashtag_to_ignore):
                pass
            else:
            #pass
                if tag["text"] in hashtag_dict:
                    #print("yeah")    
                    hashtag_dict[tag["text"]] = hashtag_dict[tag["text"]] + 1
                    #print(hashtag_dict)
                else:
                    hashtag_dict[tag["text"]] = 1
    print(hashtag_dict)
    A = sorted(hashtag_dict.items(), key = lambda kv:(kv[1], kv[0]))
    print(A)
    return A[-1][0]
